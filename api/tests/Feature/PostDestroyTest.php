...
class PostDestroyTest extends TestCase
{
     ...
     * 有効なリクエスト（正常）
     */
    public function test_destroy_mypost_with_valid_request_return_204(): void
    {
        // Act
        ...
        $this->assertDatabaseMissing('comments', ['post_id' => $this->post1->id]);
    }

    /**
     * Authorization無でリクエスト（異常）
     */
    public function test_destroy_mypost_without_authorization_return_401(): void
    {
        // Act
        $response = $this->withHeaders(['Accept' => 'application/json',])->delete($this->path. '1');
        // Assert
        $response->assertStatus(401);
        $this->assertDatabaseHas('posts', ['id' => $this->post1->id]);
        $this->assertDatabaseHas('comments', ['post_id' => $this->post1->id]);
    }

    /**
     * 不正なトークンでリクエスト（異常）
     */
    public function test_destroy_mypost_with_invalid_token_return_401(): void
    {
        // Arrange
     ...
     * 有効期限切れトークンでリクエスト（異常）
     */
    public function test_destroy_mypost_with_expired_token_return_401(): void
    {
        // Arrange
    ...
    public function test_destroy_others_return_403(): void
    {
        ...
        $this->assertDatabaseHas('posts', ['id' => $this->post2->id]);
    }
