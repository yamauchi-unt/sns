 ...
 * 自分の投稿取得 機能テスト
 */
class MypostIndexTest extends TestCase
{
    use RefreshDatabase;
     ...
     * 開始ページの有効なリクエスト（正常）
     */
    public function test_index_mypost_first_page_with_valid_request_return_200(): void
    {
        // Act
        ...
        $response->assertStatus(200)
                 ->assertJson([
                    'total' => 25,
                    'per_page' => 10,
                    'next_page_url' => 'http://localhost/api/myposts?page=2'])
                 ->assertJsonStructure(['data'=> [ '*' => ['id']]]);

        $this->assertEquals(26, $responseData['data'][0]['id']);
        $this->assertEquals(17, $responseData['data'][9]['id']);
    }

    /**
     * 2ページ目の有効なリクエスト（正常）
     */
    public function test_index_mypost_second_page_with_valid_request_return_200(): void
    {
        // Act
        ...
        $response->assertStatus(200)
                 ->assertJson([
                    'total' => 25,
                    'per_page' => 10,
                    'next_page_url' => 'http://localhost/api/myposts?page=3'])
                 ->assertJsonStructure(['data'=> [ '*' => ['id']]]);

        $this->assertEquals(16, $responseData['data'][0]['id']);
        $this->assertEquals(7, $responseData['data'][9]['id']);
    }

    /**
     * 最終ページの有効なリクエスト（正常）
     */
    public function test_index_mypost_last_page_with_valid_request_return_200(): void
    {
        // Act
        ...
        $this->assertEquals(6, $responseData['data'][0]['id']);
        $this->assertEquals(2, $responseData['data'][4]['id']);
    }
